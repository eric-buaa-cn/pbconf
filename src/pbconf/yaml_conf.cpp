...
inline bool OnNodeFor<int32_t>(
    ...
    if (field->is_repeated()) {
        return OnNodeForRepeated<int32_t>(node, field, parent_msg);
    } else {
        return OnNodeForSingle<int32_t>(node, field, parent_msg);
    }
}
// End int32_t
...
inline bool OnNodeFor<uint32_t>(
    ...
    if (field->is_repeated()) {
        return OnNodeForRepeated<uint32_t>(node, field, parent_msg);
    } else {
        return OnNodeForSingle<uint32_t>(node, field, parent_msg);
    }
}
// End uint32_t
...
inline bool OnNodeFor<int64_t>(
    ...
    if (field->is_repeated()) {
        return OnNodeForRepeated<int64_t>(node, field, parent_msg);
    } else {
        return OnNodeForSingle<int64_t>(node, field, parent_msg);
    }
}
// End int64_t
...
inline bool OnNodeFor<uint64_t>(
    ...
    if (field->is_repeated()) {
        return OnNodeForRepeated<uint64_t>(node, field, parent_msg);
    } else {
        return OnNodeForSingle<uint64_t>(node, field, parent_msg);
    }
}
// End uint64_t
...
static bool OnNode(
    ...
    if (field->cpp_type() == FieldDescriptor::CPPTYPE_INT32) {
        return OnNodeFor<int32_t>(node, field, parent_msg);
    }
    if (field->cpp_type() == FieldDescriptor::CPPTYPE_UINT32) {
        return OnNodeFor<uint32_t>(node, field, parent_msg);
    }
    if (field->cpp_type() == FieldDescriptor::CPPTYPE_INT64) {
        return OnNodeFor<int64_t>(node, field, parent_msg);
    }
    if (field->cpp_type() == FieldDescriptor::CPPTYPE_UINT64) {
        return OnNodeFor<uint64_t>(node, field, parent_msg);
    }
